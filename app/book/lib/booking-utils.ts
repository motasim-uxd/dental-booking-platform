import { operatoryAllowSet, operatoryDisplayLabel } from "@/lib/bookingOperatoryRules";
...
function operatoryPreferRank(operatoryId: number, serviceType?: ServiceType): number {
  if (!serviceType) return operatoryId;
  const allow = [...operatoryAllowSet(serviceType)];
  const i = allow.indexOf(operatoryId);
  return i === -1 ? 99 : i;
}

/** One pill per start time; prefer the operatory allowed for this service. */
export function mergeSlotsByStartTime(
  ...
  const sorted = [...options].sort((a, b) => {
    ...
    if (da !== 0) return da;
    const pr = operatoryPreferRank(a.operatoryId, serviceType) - operatoryPreferRank(b.operatoryId, serviceType);
    if (pr !== 0) return pr;
...
export function operatoryRoomLabel(operatoryId: number) {
  return operatoryDisplayLabel(operatoryId);
}
...
/** Human-readable label for messages (e.g. "May 23, 2026"). */
export function formatDateLabel(dateISO: string) {
  const [y, m, d] = dateISO.split("-").map(Number);
  if (![y, m, d].every(Number.isFinite)) return dateISO;
...
/** Oryx schedule rows may include date as ISO string or { year, month, day }. */
export function parseOryxSlotDateISO(row: { date?: unknown }): string | null {
  const d = row?.date;
  if (typeof d === "string") {
  ...
  if (d && typeof d === "object") {
    ...
    if ([y, mo, day].every(Number.isFinite)) {
...
export function earliestOryxSlotDateISO(rows: unknown[]): string | null {
  let best: string | null = null;
  for (const row of rows) {
    const iso = parseOryxSlotDateISO(row as { date?: unknown });
    if (!iso) continue;
